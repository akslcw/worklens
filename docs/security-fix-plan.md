...
**位置**：`EmployeeHomeView.vue`（`loadUsageView` 请求序号守卫 + 日期输入禁用）。

**问题**：快速切换日期发出并发请求，先发后至的旧响应覆盖新日期数据。

**修复方案（已实施）**：`loadUsageView` 增加请求序号（`usageViewRequestSequence`），仅最新一次请求的结果写入视图/错误/loading 状态；加载期间禁用日期输入；`loadUsageView(date?)` 支持显式日期参数（亦便于测试直驱竞态）。

**验收标准**：
- [x] 前端单测：先发 07-09（挂起）再发 07-08（即时返回）→ 视图显示 07-08 数据；晚到的 07-09 响应被忽略；加载期间日期输入为 disabled；
- [x] 前端 46/46、生产构建通过。
...
| 决策 | 结论 | 落地情况 |
|---|---|---|
| 1 · 代码签名证书 | **B：暂不购买**，先内部试用验证产品 | 已按 B 执行，README 已知限制中标注 |
| 2 · 管理员初始化 | **A：环境变量引导** | 已实现 `AdminBootstrap`（`WORKLENS_ADMIN_*`），无 MANAGER 时自动创建并强制首登改密；集成测试 2 用例 |
| 3 · k-匿名阈值 | **C：暂不处理**，维持现状 | 仅文档标注风险（README 已知限制），未加阈值 |
| 4 · 软删除清理周期 | **A：30 天** | 已实现 `RetentionScheduler`（每天 02:00 HKT，`WORKLENS_RETENTION_DAYS` 可调），物理清理员工及其使用记录/申请/审计/孤立账号行；集成测试 1 用例 |
| 5 · 依赖安全 | 运行扫描 | `npm audit` 已清零（修复 postcss/nanoid 构建链漏洞）；后端 OWASP dependency-check 结果见扫描报告 |
