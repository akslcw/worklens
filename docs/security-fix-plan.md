...
## H1 · 调度同刻竞争：周/月报告可能缺最后一天 [ ]

**位置**：`worklens_backend/src/main/java/com/su/worklens_backend/scheduler/ReportGenerationScheduler.java:21-37`。
...
**修复方案**：
1. 错峰执行：日报保持 23:55；周报改为次日 00:30（cron `0 30 0 * * MON`）；月报改为次日 01:00（cron `0 0 1 1-2 * *` + 代码判断"昨天是月末"），保证先日报后周报、先周报后月报；
2. 把 cron 默认值与环境变量默认值同步修改（`compose.yml`、README 表）；
3. 可选加固：给调度线程池配置 1 个线程并给周/月任务加"前置任务当日完成"的防御性检查。

**验收标准**：
- `ReportGenerationSchedulerTests` 新增用例：模拟周日 23:55 同时触发，断言执行顺序为 日→周→月（或断言周任务等待日报完成）；
- 集成测试：周报聚合覆盖完整 7 天日报。
